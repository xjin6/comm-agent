...
## [skill-causal-inference] - 2026-04-17 (3)

### Added

- Bundled R template scripts in `scripts/`: `causal_did.R`, `causal_psm.R`, `causal_psw.R`, `causal_iv.R`, `causal_rdd.R`
- Each script is fully self-contained with ALL CAPS placeholders, inline dependency install, and all outputs (Word tables + PNG figures)
- SKILL.md Step 5 updated to reference bundled templates and copy-then-customize workflow

---

## [skill-causal-inference] - 2026-04-17 (2)

### Added

- RDD (Regression Discontinuity Design) support: McCrary density test, rdrobust bandwidth selection, sharp vs fuzzy RDD, placebo cutoff robustness checks
- DID full output suite: descriptive stats table, 5-column progressive main regression table, treatment/control trends plot, event-study plot, placebo/robustness table — all with code templates
- PSM/PSW detailed implementation notes: MatchIt caliper matching, Love plot, overlap plot, WeightIt ATE weights, weight trimming

### Fixed

- README.md: updated output file list to .docx, added missing R dependencies (fixest, flextable, officer, rdrobust, rddensity, cobalt), added RDD to methods table, bumped to v0.2.0
- SKILL.md frontmatter description updated to include RDD
- CLAUDE.md and root README.md skill description updated to include RDD and Word table output

---

## [skill-causal-inference] - 2026-04-17
### Added
- New skill: causal inference toolkit supporting DID, PSM, PSW, IV
- Feasibility diagnosis, data checks, three-line tables, diagnostic plots
- Default R workflow; optional Python workflow
